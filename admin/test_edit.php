...
$test = $conn->query("SELECT * FROM tests WHERE id = $id")->fetch_assoc();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("
        UPDATE tests SET topic_id = ?, is_final = ?, passing_score = ?
        WHERE id = ?
    ");
    $stmt->bind_param('iiii', $topic_id, $is_final, $passing_score, $id);
    $stmt->execute();
...
<form method="post" class="form form--admin">
    ...
    <div class="form__field">
        <label>
            <input type="checkbox" name="is_final" <?= $test['is_final'] ? 'checked' : '' ?>>
            Итоговый экзамен
        </label>
    </div>
    ...
    <div class="form__field">
        ...
        <select name="topic_id" class="form__select">
            ...
            <?php while ($t = $topics->fetch_assoc()): ?>
                <option value="<?= $t['id'] ?>" <?= $test['topic_id'] == $t['id'] ? 'selected' : '' ?>>
    ...
    <div class="form__field">
        <label class="form__label">Проходной %</label>
        <input type="number" name="passing_score" class="form__input"
               value="<?= $test['passing_score'] ?>" required>
    </div>
